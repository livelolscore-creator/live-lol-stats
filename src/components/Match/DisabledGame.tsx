...
export function DisabledGame({ firstWindowFrame, gameMetadata, gameIndex, eventDetails }: Props) {
    ...
    if (gameMetadata?.blueTeamMetadata?.participantMetadata?.[0]?.summonerName) {
        ...
        if ((summonerName[0] && summonerName[0].startsWith(redTeam.code)) || gameMetadata.blueTeamMetadata.esportsTeamId !== blueTeam.id) {
            blueTeam = redTeam;
            redTeam = auxBlueTeam;
        }
    }
    ...
    const goldDiff = firstWindowFrame.blueTeam.totalGold - firstWindowFrame.redTeam.totalGold;
    const goldDiffAbs = Math.abs(goldDiff);
    const goldAheadTeam = goldDiff > 0 ? "blue" : "red";
                ...
                <div className="live-game-stats-header">
                    <div className="live-game-stats-header-team-images">
                        <div className="live-game-card-team">
                            {goldDiff > 0 && <span className="team-gold-diff team-gold-diff-top gold-diff-ahead">+{goldDiffAbs.toLocaleString()}</span>}
