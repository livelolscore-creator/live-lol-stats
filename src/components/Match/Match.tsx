...
export function Match({ match }: any) {
    ...
    useEffect(() => {
        ...
        function getEventDetails(gameIndex: number) {
            getEventDetailsResponse(matchId).then(response => {
                ...
                if (eventDetails === undefined) {
                    setError("Match data not available. This match may have ended or data is no longer accessible.");
                    return;
                }
        ...
        function getGameIndex(eventDetails: EventDetails): number {
            let gameIndexMatch = window.location.href.match(/game-index\/(\d+)/)
            if (gameIndexMatch) {
                console.log("Using game index from URL:", parseInt(gameIndexMatch[1]));
                return parseInt(gameIndexMatch[1]);
            }
            let inProgressGame = eventDetails.match.games.find(game => game.state === "inProgress");
            if (inProgressGame) {
                console.log("Found in-progress game:", inProgressGame.number);
                setGameIndex(inProgressGame.number);
                return inProgressGame.number;
            }
            let newGameIndex = getNextUnstartedGameIndex(eventDetails)
            console.log("Next game index:", newGameIndex);
            setGameIndex(newGameIndex)
            return newGameIndex
        }
    ...
    useEffect(() => {
        function handleCopyChampionNames() {
            if (!metadata) return
        ...
        $(`.copy-champion-names`).on(`click`, handleCopyChampionNames);
    ...
    function getStreamDropdown(eventDetails: EventDetails) {
        ...
        let dropdown = streamsOrVods.map((stream, index) => {
            ...
            return <option key={index} value={stream.parameter} data-provider={stream.provider} data-parameter={stream.parameter}>{streamString}</option>
        })
    ...
    if (firstWindowFrame !== undefined && lastWindowFrame !== undefined && lastDetailsFrame !== undefined && metadata !== undefined && eventDetails !== undefined && currentGameOutcome !== undefined && scheduleEvent !== undefined && gameIndex !== undefined && items !== undefined && runes !== undefined) {
    ...
    } else if (eventDetails !== undefined && scheduleEvent !== undefined) {
        const team1 = eventDetails.match.teams[0];
        const team2 = eventDetails.match.teams[1];
        const matchDate = new Date(scheduleEvent.startTime).toISOString().split('T')[0];
        const leagueName = eventDetails.league.name;
        const seoTitle = `${team1.name} vs ${team2.name} - ${leagueName} Live Score | ${matchDate}`;
        const seoDescription = `Watch live ${team1.name} vs ${team2.name} ${leagueName} match. Get real-time stats, results, and game analysis. ${team1.code} vs ${team2.code} esports.`;
        ...
        if (metaKeywords) metaKeywords.setAttribute("content", `${team1.name}, ${team2.name}, ${team1.code}, ${team2.code}, ${leagueName}, LoL esports, live match`);
        ...
        return (
            <div className="loading-game-container">
                <div>
                    <h3 style={{color: '#C8A84B'}}>Unable to Load Match</h3>
                    <p style={{color: '#7A8599'}}>{error}</p>
                    <p style={{color: '#4A5568', marginTop: '1rem'}}>This match may have ended or data is no longer available from the LoL Esports API.</p>
                </div>
            </div>
        )
    } else {
