...
import { generateMatchSlug } from "../../utils/LoLEsportsAPI";
...
export function EventCard({ scheduleEvent, clickable = true }: Props) {
    const matchSlug = generateMatchSlug(
        scheduleEvent.match.teams[0].code,
        scheduleEvent.match.teams[1].code,
        new Date(scheduleEvent.startTime).toISOString()
    );
    const matchUrl = `/live/${scheduleEvent.match.id}/${matchSlug}`;
    const card = (
