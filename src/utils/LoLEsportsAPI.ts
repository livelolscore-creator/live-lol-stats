...
export function getScheduleResponse(pageToken?: string, leagueId?: string) {
    const params: any = { "hl": "en-US" };
    if (pageToken) {
...
export function getMatchDetailsResponse(matchId: string) {
    return axios.get(`${API_URL_PERSISTED}/getEventDetails`, {
        params: {
            "hl": "en-US",
            "id": matchId,
...
export function generateMatchSlug(team1Code: string, team2Code: string, date: string): string {
    const formattedDate = new Date(date).toISOString().split('T')[0];
    return `${team1Code.toLowerCase()}-vs-${team2Code.toLowerCase()}-${formattedDate}`;
}

export function parseMatchSlug(slug: string): { team1: string; team2: string; date: string } | null {
    const match = slug.match(/^([a-z0-9]+)-vs-([a-z0-9]+)-(\d{4}-\d{2}-\d{2})$/);
    if (match) {
        return {
...
export function getMatchUrl(matchId: string, team1Code: string, team2Code: string, date: string): string {
    const slug = generateMatchSlug(team1Code, team2Code, date);
    return `/live/${matchId}/${slug}`;
}

export function isNumericMatchId(value: string): boolean {
    return /^\d+$/.test(value);
}
